...
\end{align}
Całkowanie po sferze wokół punktu $\vec{p}$ zostało zredukowane do całkowania po półsferze wyznaczonej przez kierunek normalny w $\vec{p}$. Niech $k_d(\vec{p}) = \frac{\mathrm{d}\Phi_o}{\mathrm{d}\Phi_i}$ będzie stopniem odbicia, wtedy:
\begin{equation}
f_s(\vec{p}) = \frac{k_d(\vec{p})}{\pi}
\end{equation}

\subsubsection{Model Oren-Nayar światła rozproszonego}
\subsubsection{Model Phonga oraz Blinna-Phonga dla odbić kierunkowych}
\subsubsection{Model Torrance-Sparrow dla odbić kierunkowych}
