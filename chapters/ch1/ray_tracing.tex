...
\addimage{chapters/ch1/img/triangle.png}{scale=0.8}{Trójkąt po rasteryzacji. Siatka kwadratów odpowiada niepodzielnej siatce pikseli obrazu. Wierzchołki zostały oznaczone {\color{red}czerwonymi} punktami i połączone ze sobą. Z uwagi na fakt, iż piksele są elementami dyskretnymi, odpowiednie algorytmy muszą zdecydować czy dana płaszczyzna w odpowiednio dużym stopniu zajmuje miejsce odpowiadające danemu pikselowi, w efekcie powstają postrzępione granice obiektów. Dla pikseli znajdujących się wewnątrz ({\color{blue}niebieskie}) należy dokonać interpolacji atrybutów}{Trójkąt po rasteryzacj}{ch1:img:triangleRaster}
...
Powyższy opis procesu rasteryzacji, mimo iż znacznie uproszczony w stosunku do wykorzystywanych obecnie potoków przetwarzania~\cite{OPENGL_SUPERBIBLE}, zawiera sedno całej operacji i pozwala wyróżnić kilka jej najważniejszych cech.
\begin{enumerate}
...
\addimage{chapters/ch1/img/spheres.png}{width=0.6\textwidth}{Triangulacja obiektów na przykładzie sfery - im więcej podziałów powierzchni tym gładszy obiekt, jednak czas renderowania dłuższy}{Triangulacja obiektów na przykładzie sfery}{ch1:img:shpereTriangulation}
...
\addimage{chapters/ch1/img/reflRefrHuygens.png}{width=0.7\textwidth}{Konstrukcja fali odbitej i załamanej na bazie zasady Huygensa. Fala {\color{red}padająca} na granicę dwóch ośrodków $\Pi$ pod kątem $\varepsilon$ do normalnej $\vec{n}$ powoduje powstanie fali {\color{green}odbitej}, takiej że $\gamma = \varepsilon$ oraz {\color{blue}załamanej}, przy czym kąt załamania $\delta\neq\varepsilon$ i zależy od relacji między prędkością propagacji fali w obu ośrodkach rozdzielonych przez $\Pi$}{Konstrukcja fali odbitej i załamanej na bazie zasady Huygensa}{ch1:img:reflRefrHuygens}
...
\addimage{chapters/ch1/img/continuity_fin.png}{width=0.7\textwidth}{Ciągłość pola elektrycznego na granicy ośrodków. Wymiar w kierunku normalnym do $\Pi$ $\partial h$ obu obiektów, względem których następuje całkowanie jest nieskończenie mały w szczególności w porównaniu z $|\mathrm{d}\vec{l}|$}{Ciągłość pola elektrycznego na granicy ośrodków}{ch1:img:Continuity}
...
\addimage{chapters/ch1/img/reflectance_transmitance.png}{scale=0.35}{Odbicie i załamanie strumienia padającego promieniowania $I_i$ na powierzchni $\Pi$ (rzut na płaszczyznę padania). Strumień padającego promieniowania $I_i$ przypada na pewną powierzchnię $A$. Z uwagi na fakt, iż $\theta_i = \theta_r \neq \theta_t$ pole przekroju poprzecznego strumieni $I_i$ oraz $I_t$ jest różne}{Odbicie i załamanie strumienia padającego promieniowania}{ch1:img:reflectance_transmitance}
...
\addimage{chapters/ch1/img/im_angular_r.png}{width=0.85\textwidth}{Reflektancja dla dobrego przewodnika dla obu typów polaryzacji w funkcji kąta padania, $n_w = \eta + ik = 1,66 + 2i\in\mathbb{C}$. Obecność czynnika tłumiącego sprawia, iż tylko niewielka objętość przewodnika odczuwa obecność padającej fali, dzięki czemu większa część promieniowania (w porównaniu z dielektrykami) odbija się od powierzchni nawet, gdy $\theta_i = 0^\circ$ - jest to przyczyna tzw. połysku metalicznego }{Reflektancja dla dobrego przewodnika dla obu typów polaryzacji w funkcji kąta padania}{ch1:img:im_angular_r}

\addimage{chapters/ch1/img/gold_nk.png}{scale=0.30}{Współczynniki $\eta$ oraz $k$ dla złota dla optycznych długości fal $\lambda$. Obie wartości wykazują silną zależność od długości fali $\lambda$. Na podstawie przebiegu ich zmienności można wyjaśnić, dlaczego metal ten posiada swój charakterystyczny kolor. Wykres zaczerpnięty z~\cite{PBRT}}{Współczynniki $\eta$ oraz $k$ dla złota dla optycznych długości fal}{ch1:img:gold_nk}
...
\end{itemize}
\addimage{chapters/ch1/img/Reflection_terms.png}{width=1.0\textwidth}{Rodzaje odbicia od powierzchni. Odbicie rozproszone a), kierunkowe rozproszone b) i jego szczególny przypadek odbicie zwierciadlane c)}{Rodzaje odbicia od powierzchni}{ch1:img:Reflection_terms}
...
Zdecydowana większość rzeczywistych powierzchni, mimo że wydaje się być gładka w rzeczywistości nie jest. Badanie powierzchni na przykład przy pomocy mikroskopii sił atomowych~(ang.~\textit{Atomic Force Microscopy}, AFM) pokazuje, że  materiał w rzeczywistości posiada pewne mikronierówności - te zaś wpływają na to jak dany materiał jako całość rozprasza światło.
\addimage{chapters/ch1/img/afm_surface_topology.png}{scale=0.3}{Topologia rzeczywistej powierzchni uzyskana techniką mikroskopii sił atomowych~(AFM)~\cite{SurfaceImaging}. W znacznej części przypadków makroskopowo płaskie obiekty posiadają skomplikowane topologie, które między innymi wpływają na makroskopowy sposób oddziaływania ze światłem}{Topologia rzeczywistej powierzchni}{ch1:img:afm_surface_topology}
...
\addimage{chapters/ch1/img/cose.png}{scale=0.65}{Zależność funkcji rozbłysku od parametru skupienia $e$. Im większa wartość $e$ tym rozkład staje się silniej skupiony wokół $\alpha=0^\circ$}{Zależność funkcji rozbłysku od parametru skupienia $e$}{ch1:img:cose}
...
\addimage{chapters/ch1/img/Specular_reflection_blinn.png}{scale=0.5}{Modelowanie odbić w modelu Blinna-Phonga. Kąt $\alpha$ definiowany jest jako kąt między między $\vec{\omega_h} = \frac{\vec{l} + \vec{\omega_o}}{|\vec{l} + \vec{\omega_o}|}$ a normalną $\vec{n}$ do powierzchni }{Modelowanie odbić w modelu Blinna-Phonga}{ch1:img:Specular_reflection_blinn}
...
\addimage{chapters/ch1/img/Torrance_Sparrow.png}{scale=0.55}{Geometria odbicia w modelu Torrance'a-Sparrowa. Podobnie jak w przypadku modelu Blinna-Phonga duże znaczenie posiada wektor połówkowy $\vec{\omega_h}$, który jest wektorem normalnym do mikrościanki}{Geometria odbicia w modelu Torrance'a-Sparrowa}{ch1:img:Torrance_Sparrow}
