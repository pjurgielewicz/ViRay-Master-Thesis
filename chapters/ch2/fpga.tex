...
\begin{center}
\addimage{chapters/ch2/img/bd_example.png}{scale=0.41}{Schemat blokowy prostego przykładowego systemu przetwarzania danych. Sercem systemu w tym przypadku jest mikroprocesor Microblaze  wyposażony w możliwość sprawdzania poprawności przetwarzania za pomocą \textit{Microblaze Debug Module}. Drugim istotnym elementem jest generator interfejsu do pamięci zewnętrznej \textit{DDR4 SDRAM} - dzięki niemu możliwe jest zapisywanie i odczyt danych przez Microblaze z i do zewnętrznej względem układu FPGA pamięci RAM. Jednak, aby Microblaze mógł się komunikować z modułem zarządzającym pamięcią zewnętrzną są one połączone ze sobą za pomocą \textit{AXI Interconnect}. Na tej samej zasadzie z mikroprocesorem połączone są licznik czasu \textit{AXI Timer} oraz moduł do komunikacji zewnętrznej \textit{AXI Uartlite}~(wykorzystywany najczęściej do wypisywania tekstu na terminalu). W systemie tym występują dwa sygnały zegarowe: pierwszy (\textit{default\_sysclk1\_300}) wpływa na szybkość pracy interfejsu pamięci zewnętrznej, zaś drugi generowany przez \textit{Clocking Wizard} decyduje m. in o wydajności pracy mikroprocesora.  }{Schemat blokowy prostego przykładowego systemu przetwarzania danych}{ch2:img:bd_example}
\end{center}
